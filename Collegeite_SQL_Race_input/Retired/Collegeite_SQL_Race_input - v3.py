...
class TimeEntry(tk.Entry):
    """Entry widget for race time input with smart parsing similar to Race Ranker."""
    ...
    def _normalize(self, *_):
        ...
        try:
            ...
            messagebox.showerror("Invalid Time", "Enter time as digits (e.g., 704 for 7:04.000) or mm:ss.fff format")
            self.focus_set()
...
class RowingDatabaseApp:
    ...
    def _create_event_tab(self):
        ...
        # Add scrollbar
        events_scrollbar = ttk.Scrollbar(tree_frame, orient='vertical', command=self.events_tree.yview)
    ...
    def _create_entry_tab(self):
        ...
        self.entries_tree.column('School', width=200, anchor='center')
        self.entries_tree.column('Boat Class', width=100, anchor='center')
        self.entries_tree.column('Conference', width=150, anchor='center')
        
        # Create a frame to center the treeview
        entries_tree_frame = tk.Frame(entries_frame)
        entries_tree_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        # Add scrollbar
        entries_scrollbar = ttk.Scrollbar(entries_tree_frame, orient='vertical', command=self.entries_tree.yview)
    ...
    def _create_results_tab(self):
        ...
        # Center the headers
        headers_inner_frame = tk.Frame(headers_frame)
        headers_inner_frame.pack(anchor='center')
        
        tk.Label(headers_inner_frame, text="Pos", font=FONT_LABEL, width=5).grid(row=0, column=0, padx=2)
        tk.Label(headers_inner_frame, text="", font=FONT_LABEL, width=3).grid(row=0, column=1, padx=2)  # For arrows
        tk.Label(headers_inner_frame, text="School", font=FONT_LABEL, width=20).grid(row=0, column=2, padx=2)
        tk.Label(headers_inner_frame, text="Lane", font=FONT_LABEL, width=6).grid(row=0, column=3, padx=2)
        tk.Label(headers_inner_frame, text="Time (mm:ss.fff)", font=FONT_LABEL, width=15).grid(row=0, column=4, padx=2)
        tk.Label(headers_inner_frame, text="Margin", font=FONT_LABEL, width=10).grid(row=0, column=5, padx=2)
        
        # Scrollable results area
        canvas_frame = tk.Frame(self.results_frame)
        canvas_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        canvas = tk.Canvas(canvas_frame)
        scrollbar_results = tk.Scrollbar(canvas_frame, orient="vertical", command=canvas.yview)
    ...
    def _setup_results_form(self, entries):
        ...
        for i, (entry_id, school_name, boat_class, conference) in enumerate(entries):
            ...
            row_frame.pack(anchor='center', pady=2)  # Center each row
            ...
            # Time (with improved smart parsing)
